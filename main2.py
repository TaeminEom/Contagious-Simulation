...
class Student:
    ...
    def randSetClass(self, randList):
        self.pos = [self.groupsPos[randList[0]][0] + 25 + randList[1] % 6 * 30, self.groupsPos[randList[0]][1] + 40 + randList[1] // 6 * 25]
...
for try20 in range(30):
    ...
    for g in range(8):
        for n in range(24):
            randList.append([g, n])
    rd.shuffle(randList)
    randList = np.array(randList)
    randList = randList.reshape(8, 24, 2)
    ...
    while not done:
        ...
        if start > 300 and t < 1000 and 2000 < t and breakTime == True:
            start = 0
            breakTime = False
            for g in range(8):
                for n in range(24):
        ...
        if start > 300 and (1000 < t or t < 2000) and breakTime == True:
            start = 0
            breakTime = False
            for g in range(8):
                for n in range(24):
                    student[g][n].randSetClass(randList[g][n])
                    student[g][n].corridor = False
